...
MAX_QUEUE_LENGTH = 20
MAX_WORKERS = 3 # Maximum number of threads you can process at a time
...
# Letter mapping
LETTER_MAPPING = {'a': A, 'b': B, 'c': C, 'd': D, 'e': E, 'f': F, 'g': G, 'h': H, 'i': I, 'j': J, 'k': K, 'l': L, 'm': M, 'n': N, 'o': O, 'p': P, 'q': Q, 'r': R, 's': S, 't': T, 'u': U, 'v': V, 'w': W, 'x': X, 'y': Y, 'z': Z}
LETTER_SET = frozenset(LETTER_MAPPING.keys())
...
def handle_message(message):
    try:
        ...
        if msg == "left":
            HoldAndReleaseKey(LEFT_ARROW, 0.1)
        if msg == "right":
            HoldAndReleaseKey(RIGHT_ARROW, 0.1)
        if msg == "up":
            HoldAndReleaseKey(UP_ARROW, 0.1)
        if msg == "down":
            HoldAndReleaseKey(DOWN_ARROW, 0.1)

        if msg in LETTER_SET:
            HoldAndReleaseKey(LETTER_MAPPING[msg], 0.1)
